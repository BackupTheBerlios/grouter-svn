...
public class NodeConfigFactory
{
    ...
    private static NodeConfig createNodeConfig(NodeType nodeType, boolean isRelative, String relativePath)
    {
        ...
        OutFolderConfig outFolderConfig = new OutFolderConfig(new File( nodePath + nodeType.getOutFolderConfig().getOutFolderPath()));
        BatchReadConfig batchReadConfig = new BatchReadConfig(nodeType.getInFolderConfig().getBatchReadConfig().getIsBatchOn(), nodeType.getInFolderConfig().getBatchReadConfig().getBatchSize(), nodeType.getInFolderConfig().getBatchReadConfig().getBatchThreshold());
        InFolderConfig inFolderConfig = new InFolderConfig(new File(nodePath + nodeType.getInFolderConfig().getInFolderPath()), nodeType.getInFolderConfig().getPollIntervallMilliSeconds(), nodeType.getInFolderConfig().getSkipFirstblankline(), batchReadConfig);
