...
public class HttpTestMwmlPortalWithCookie extends HttpUnitTestCase {
	...
	protected void setUp() throws Exception {
		super.setUp();
		client = this.getClient("SonyEricssonT610i");
	}
	...
	public void testPortalWithCookie() throws MalformedURLException, IOException, SAXException {
		...
		response = get$CharacterAssertedResponse(client, "http://localhost/mwml/portal"); 
		writeResponseToFile(response, "c:\\tmp\\testMwmlPortalWithCookie.html");

		String text = response.getText();
		
		String str = "Choose your location";
		boolean hasChooseYourLocation = text.indexOf(str)!=-1;
		assertFalse("With preferred locale in cookie, page should not contain "+str, hasChooseYourLocation);
		
		String ringtones = "Ring tones";
		boolean hasRingtones = text.indexOf(ringtones)!=-1;
		assertTrue("With preferred locale in cookie, page should contain Ring tones (overview1)", hasRingtones);
	}
...
*/	
	...
	public void testPortalWithoutCookie() throws MalformedURLException, IOException, SAXException {
		response = get$CharacterAssertedResponse(client, "http://localhost/mwml/portal"); 
		writeResponseToFile(response, "c:\\tmp\\testMwmlPortalWithoutCookie.html");
		...
		String ringtones = "Ring tones";
		boolean hasRingtones = text.indexOf(ringtones)!=-1;
		assertFalse("Without preferred locale in cookie, page should not contain Ring tones (overview1)", hasRingtones);
	}
